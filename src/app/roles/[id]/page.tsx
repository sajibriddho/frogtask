...
export default function EditRolePage() {
    ...
    const fetchInitial = async () => {
      try {
        ...
        if (!roleData.success || !roleData.data) {
          toast.error(roleData.error || "Role not found");
          router.push("/roles");
          return;
        }
        ...
        const permissionsData = await parseJsonSafe<{
          success: boolean;
          data: PermissionRow[];
          error?: string;
        }>(permissionsRes);
        ...
        if (permissionsData.success) {
          setPermissionTree(buildPermissionTree(permissionsData.data ?? []));
        } else {
          toast.error(permissionsData.error || "Failed to load permissions");
          setPermissionTree([]);
        }

        const rolePermissionsData = await parseJsonSafe<{
          success: boolean;
        ...
        if (rolePermissionsData.success) {
          setSelectedIds(
            new Set(rolePermissionsData.data?.permission_ids ?? []),
          );
        } else {
          toast.error(
  ...
  const onSubmit = async (values: RoleFormValues) => {
    ...
    try {
      ...
      if (!roleData.success) {
        toast.error(roleData.error || "Failed to update role");
        return;
      }
      ...
      if (!rolePermissionData.success) {
        toast.error(rolePermissionData.error || "Failed to update permissions");
        return;
      }
